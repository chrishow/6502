...
let value = cpu.flagsToByte();
assertEquals(value, 255);

console.log('Test PHP'); 
cpu.memory.hexLoad(0x0600, '08'); // PHP
cpu.registers.pc = 0x0600;
cpu.steps(3);
assertEquals(cpu.registers.sp, 0xFE);
assertEquals(cpu.memory.readByte(0x01FF), 0xFF);


console.log('Test PLP'); 
cpu.memory.hexLoad(0x0600, '28'); // PLP
cpu.registers.sr.b = 0;
cpu.registers.pc = 0x0600;
cpu.steps(13);
assertEquals(cpu.registers.sp, 0xFF);
...
console.log('Test ROR A'); 
cpu = new CPU;
cpu.memory.hexLoad(0x0600, 'a9 fa 6a'); // LDA #$FA, ROR
cpu.registers.pc = 0x0600;
cpu.steps(10);
assertEquals(cpu.registers.a, 0x7D);
assertEquals(cpu.registers.sr.n, 0);
assertEquals(cpu.registers.sr.c, 0);

console.log('Test ROR ABS'); 
cpu = new CPU;
cpu.memory.hexLoad(0x0600, 'a9 fa 85 a0 66 a0'); // LDA #$FA, STA $A0, ROR $A0
cpu.registers.pc = 0x0600;
cpu.steps(10);
